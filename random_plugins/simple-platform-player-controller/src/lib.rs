...
const MAX_SPEED: f32 = TILE_SIZE_PIXELS * 3.;
const ACCELERATION: f32 = 600.;
const JUMP_SPEED: f32 = 120.;
const AERIAL_X_DAMPENING_FACTOR: f32 = 0.2;
...
fn keyboard_input_system(
    ...
    player: Query<(Option<&Grounded>, &DoubleJumped), With<Player>>,
) {
    let mut direction = Vec2::ZERO;
    let (grounded, double_jumped) = player.iter().next().unwrap();
...
fn player_move_action_system(
    ...
    for (entity, mut linear_velocity, grounded, mut double_jumped) in player_velocity.iter_mut() {
        for movement_action in movement_events.read() {
            match movement_action {
                MovementAction::Horizontal(dir) => {
                    let factor = if grounded.is_none() {
                        0.1
                    } else {
                        1.
                    };
...
fn grounded_system(mut commands: Commands, mut query: Query<(Entity, &ShapeHits, &mut DoubleJumped), With<Player>>) {
...
fn movement_dampening_system(
    ...
    for (mut velocity, dampening, grounded, double_jumped) in &mut query {
        if grounded.is_none() {
